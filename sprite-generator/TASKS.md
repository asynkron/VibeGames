...
1. Refactor `createBaseConfig` in `sprite-generator/game.js` to return a single, view-agnostic spaceship schema (e.g., keep the existing top-down structure as the canonical definition). Remove the conditional that builds separate `createTopDownConfig` vs. `createSideViewConfig`; instead, generate any additional metadata needed for the side projection alongside the shared body/wing/engine fields.
