...
export function drawSideAntenna(root, config, geometry, axis) {
  ...
  const beacon = createSvgElement("circle", {
    attrs: {
      cx: tipX,
      cy: tipY - antenna.beaconRadius * 0.2,
      r: antenna.beaconRadius,
...
export function drawSideThrusters(root, config, geometry, axis) {
  ...
  for (let i = 0; i < thruster.count; i += 1) {
    ...
    const housing = createSvgElement("rect", {
      attrs: {
        x: tailX - thruster.nozzleLength - housingWidth,
        y: y - housingHeight / 2,
        width: housingWidth,
        height: housingHeight,
        rx: thruster.radius * 0.4,
    ...
    const nozzle = createSvgElement("rect", {
      attrs: {
        x: tailX - thruster.nozzleLength,
        y: y - thruster.radius * 0.75,
        width: thruster.nozzleLength,
        height: thruster.radius * 1.5,
        rx: thruster.radius * 0.3,
    ...
    const flame = createSvgElement("polygon", {
      ...
      classList: ["thruster-flame", "thruster-flame--horizontal"],
    });
...
export function drawSideWeapons(root, config, geometry, axis) {
  ...
  if (armament.mount === "wing" && wing?.enabled && armament.hardpoints?.length) {
    ...
    armament.hardpoints.forEach((hardpoint) => {
      ...
      const group = createSvgElement("g", { classList: "wing-ordnance" });
  ...
  for (let i = 0; i < armament.barrels; i += 1) {
    ...
    const housing = createSvgElement("rect", {
      attrs: {
        x: noseX - housingWidth,
        y: y - armament.housingHeight / 2,
        width: housingWidth,
        height: armament.housingHeight,
    ...
    const barrel = createSvgElement("line", {
      attrs: {
        ...
        y1: y,
        x2: noseX + 2,
        y2: y - armament.housingHeight * 0.1,
        stroke: partStroke("weapons", palette.trim),
        "stroke-width": 2.2,
        "stroke-linecap": "round",
      },
...
export function drawSideLights(root, config, geometry, axis) {
  ...
  if (lights.nose) {
    const noseLight = createSvgElement("circle", {
      attrs: {
        cx: noseX - 4,
        cy: centerY - profile.noseHeight * 0.1,
        r: 2.6,
        fill: partColor("lights", palette.glow),
        opacity: 0.85,
      },
  ...
  if (lights.dorsal) {
    ...
    const dorsal = createSvgElement("circle", {
      attrs: {
        cx: axis.percentToSideX(dorsalPercent),
        cy: centerY - profile.dorsalHeight - 6,
        r: 2.2,
  ...
  if (lights.intake && profile.intakeHeight > 0) {
    ...
    const intake = createSvgElement("circle", {
      attrs: {
        cx: axis.percentToSideX(intakePercent),
        cy: centerY + profile.ventralDepth - 6,
        r: 2.4,
  ...
  const tailBeacon = createSvgElement("circle", {
    attrs: {
      cx: tailX,
      cy: centerY - profile.tailHeight * 0.3,
      r: 2,
